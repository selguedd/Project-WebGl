...
const main = function (resources) {
  ...
  const groundColor = new THREE.Vector3(0.7, 0.7, 0.7);
  const streetColor = new THREE.Vector3(0.5, 0.5, 0.5);
  ...
  const texture_city_building_2 = (new THREE.TextureLoader()).load( "img/city_building_2.jpg" )
  const texture_city_house = (new THREE.TextureLoader()).load( "img/city_house.png" )
  const texture_car_light = (new THREE.TextureLoader()).load( "img/car_white.png" )
  ...
  function addSkyBox() {
    ...
    loader.setPath('img/cubemaps/Citadella2/');
  ...
  function create_Textured_Block(width, depth, color1, color2, color3, texture_chosen) {
  ...
  function createBlock(width, depth, color1, color2, color3) {
    const geometry = new THREE.BoxGeometry(width, height * getRandWithLimits(0.2, 1), depth);
    //const geometry = new THREE.CylinderGeometry(30, 30, height * getRandWithLimits(0.2, 1));
    var uvPixel = 0.0;
    geometry.faceVertexUvs[0][4][0]=new THREE.Vector2(uvPixel,uvPixel);
    ...
    const material = getColoredShader(new THREE.Vector3(color1, color2, color3));
    const block = new THREE.Mesh(geometry, material);
    return block;
  }
  ...
  function fillCellWithRandomBlocks(cell) {
    ...
    if (cell.position['x'] < 4000 && cell.position['x'] > 2000 && cell.position['z'] > -4000 && cell.position['z'] < -2000){
      ...
      const r_value = getRandomIntInclusive(1, 2)
      if (r_value == 1){
        texture_chosen = texture_city_building
      } else {
        texture_chosen = texture_city_building_2
      }
      ...
      const block1 = create_Textured_Block(dimX1, dimY1, color1, color2, color3, texture_chosen);
      const block4 = create_Textured_Block(dimX2, dimY2, color1, color2, color3, texture_chosen);
      ...
      const block1 = create_Textured_Block(dimX1, dimY1, color1, color2, color3, text_block_1);
      const block2 = create_Textured_Block(dimX2, dimY1, color1, color2, color3, text_block_2);
      const block3 = create_Textured_Block(dimX1, dimY2, color1, color2, color3, text_block_3);
      const block4 = create_Textured_Block(dimX2, dimY2, color1, color2, color3, text_block_4);
  ...
  */
  ...
  function createLight(no, direction) {
    ...
    const geometry = new THREE.BoxGeometry(carRadius, carRadius, carRadius);
